// app/components/VisitReport.tsx
'use client';
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      if (uniqueMeds.length > 0) {
        ...
        pdf.text('No medications logged.', margin + 2, y + 3);
        y += 10;
      }

      // === TIMING NOTES (Educational) ===
      if (timingHints.length > 0) {
        pdf.setFontSize(12);
        pdf.setFont('helvetica', 'bold');
        ensureSpace(12);
        pdf.text('Timing Notes (Educational)', margin, y);
        y += 6;
        ...
        timingHints.forEach((hint) => {
          ...
          const lines = pdf.splitTextToSize(hint.message, pageWidth - margin * 2 - 22);
          lines.forEach((line: string, j: number) => {
            pdf.text(line, margin + 22, y + 3 + j * 4);
          });

          // Metadata for audit/debug (rule ID, window)
          pdf.setFontSize(8);
          pdf.setTextColor(120);
          const metaText = `Rule: ${hint.meta?.ruleId ?? 'n/a'}${
            hint.meta?.windowHours ? ` • Window: ${hint.meta.windowHours[0]}–${hint.meta.windowHours[1]}h` : ''
          }`;
          pdf.text(metaText, margin + 22, y + 3 + lines.length * 4 + 2);
          pdf.setTextColor(0);
          pdf.setFontSize(10);
          y += Math.max(10, lines.length * 4 + 8);
        });
      }
      ...
      pdf.setFont('helvetica', 'bold');
      ensureSpace(24);
      pdf.text('Questions for Discussion', margin, y);
      y += 6;

      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(10);
      const questions = [
        'Do these patterns suggest specific triggers to test next?',
        'Should medication timing be adjusted based on recent symptoms?',
        'Do any of these symptoms warrant further investigation?'
      ];

      questions.forEach((question, i) => {
        ensureSpace(6);
        pdf.text(`${i + 1}. ${question}`, margin + 2, y + 3);
        y += 6;
      });
      ...
      // Save PDF
      const filename = `health-report-${format(now, 'yyyy-MM-dd')}.pdf`;
      pdf.save(filename);

      const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
      toast.success(`Report downloaded!`, {
        description: `Generated in ${elapsed}s • ${filename}`
      });
